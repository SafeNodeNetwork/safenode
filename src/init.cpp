...
#include "config/safenode-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    CFlatDB<CSafenodeMan> flatdb1("mncache.dat", "magicSafenodeCache");
    flatdb1.Dump(mnodeman);
    CFlatDB<CSafenodePayments> flatdb2("mnpayments.dat", "magicSafenodePaymentsCache");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, mempoolrej, net, proxy, prune, http, libevent, tor, zmq, "
                             "safenode (or specifically: privatesend, instantsend, safenode, spork, keepass, mnpayments, gobject)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all SafeNode specific functionality (Safenodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));

    strUsage += HelpMessageGroup(_("Safenode options:"));
    strUsage += HelpMessageOpt("-safenode=<n>", strprintf(_("Enable the client to act as a safenode (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf(_("Specify safenode configuration file (default: %s)"), "safenode.conf"));
    strUsage += HelpMessageOpt("-mnconflock=<n>", strprintf(_("Lock safenodes from safenode configuration file (default: %u)"), 1));
    strUsage += HelpMessageOpt("-safenodeprivkey=<n>", _("Set the safenode private key"));
    ...
    strUsage += HelpMessageOpt("-privatesendrounds=<n>", strprintf(_("Use N separate safenodes for each denominated input to mix funds (2-16, default: %u)"), DEFAULT_PRIVATESEND_ROUNDS));
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N SXN anonymized (default: %u)"), DEFAULT_PRIVATESEND_AMOUNT));
...
std::string LicenseInfo()
{
    ...
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2014-%i The SafeNode Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that SafeNode is running in a usable environment with all
...
void InitParameterInteraction()
{
    ...
    if (GetBoolArg("-safenode", false)) {
        // safenodes must accept connections from outside
        if (SoftSetBoolArg("-listen", true))
            LogPrintf("%s: parameter interaction: -safenode=1 -> setting -listen=1\n", __func__);
    }
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("SafeNode version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. SafeNode is shutting down."));
...
#endif
    // Make sure only a single SafeNode process is using the data directory.
    ...
    try {
        ...
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SafeNode is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SafeNode is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of SafeNode") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart SafeNode to complete") << "\n";
    ...
    fSafeNode = GetBoolArg("-safenode", false);

    if((fSafeNode || safenodeConfig.getCount() > -1) && fTxIndex == false) {
        return InitError("Enabling Safenode support requires turning on transaction indexing."
    ...
    if(fSafeNode) {
        ...
        if (SoftSetBoolArg("-listen", true))
            LogPrintf("This is a safenode. Enabling listen automatically\n");

        if(!GetArg("-safenodeaddr", "").empty()) {
            // Hot safenode (either local or remote) should get its address in
            // CActiveSafenode::ManageState() automatically and no longer relies on safenodeaddr.
            return InitError(_("safenodeaddr option is deprecated. Please use safenode.conf to manage your remote safenodes."));
        }

        std::string strSafeNodePrivKey = GetArg("-safenodeprivkey", "");
        if(!strSafeNodePrivKey.empty()) {
            if(!darkSendSigner.GetKeysFromSecret(strSafeNodePrivKey, activeSafenode.keySafenode, activeSafenode.pubKeySafenode))
                return InitError(_("Invalid safenodeprivkey. Please see documenation."));

            LogPrintf("  pubKeySafenode: %s\n", CBitcoinAddress(activeSafenode.pubKeySafenode.GetID()).ToString());
        } else {
            return InitError(_("You must specify a safenodeprivkey in the configuration. Please see documentation for help."));
        }
    }

    LogPrintf("Using safenode config file %s\n", GetSafenodeConfigFile().string());

    if(GetBoolArg("-mnconflock", true) && pwalletMain && (safenodeConfig.getCount() > 0)) {
        LOCK(pwalletMain->cs_wallet);
        LogPrintf("Locking Safenodes:\n");
        uint256 mnTxHash;
        int outputIndex;
    ...
    //lite mode disables all Safenode and Darksend related functionality
    fLiteMode = GetBoolArg("-litemode", false);
    if(fSafeNode && fLiteMode){
        return InitError("You can not start a safenode in litemode");
    }
    ...
    uiInterface.InitMessage(_("Loading safenode cache..."));
    CFlatDB<CSafenodeMan> flatdb1("mncache.dat", "magicSafenodeCache");
    if(!flatdb1.Load(mnodeman)) {
        return InitError("Failed to load safenode cache from mncache.dat");
    }

    if(mnodeman.size()) {
        uiInterface.InitMessage(_("Loading safenode payment cache..."));
        CFlatDB<CSafenodePayments> flatdb2("mnpayments.dat", "magicSafenodePaymentsCache");
        if(!flatdb2.Load(mnpayments)) {
            return InitError("Failed to load safenode payments cache from mnpayments.dat");
        }
        ...
        uiInterface.InitMessage(_("Safenode cache is empty, skipping payments and governance cache..."));
    }
    ...
    // ********************************************************* Step 11c: update block tip in SafeNode modules
    ...
    safenodeSync.UpdatedBlockTip(chainActive.Tip());
    governance.UpdatedBlockTip(chainActive.Tip());

    // ********************************************************* Step 11d: start safenode-privatesend thread

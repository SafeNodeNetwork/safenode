...
CGovernanceVote::CGovernanceVote(CTxIn vinSafenodeIn, uint256 nParentHashIn, vote_signal_enum_t eVoteSignalIn, vote_outcome_enum_t eVoteOutcomeIn)
    : fValid(true),
      fSynced(false),
      nVoteSignal(eVoteSignalIn),
      vinSafenode(vinSafenodeIn),
...
bool CGovernanceVote::Sign(CKey& keySafenode, CPubKey& pubKeySafenode)
{
    // Choose coins to use
    ...
    std::string strError;
    std::string strMessage = vinSafenode.prevout.ToStringShort() + "|" + nParentHash.ToString() + "|" +
...
bool CGovernanceVote::IsValid(bool fSignatureCheck) const
{
    ...
    if(!infoMn.fInfoValid) {
        LogPrint("gobject", "CGovernanceVote::IsValid -- Unknown Safenode - %s\n", vinSafenode.prevout.ToStringShort());
        return false;
    }
    ...
    if(!darkSendSigner.VerifyMessage(infoMn.pubKeySafenode, vchSig, strMessage, strError)) {
...
{
    bool fResult = ((vote1.vinSafenode == vote2.vinSafenode) &&
...
bool operator<(const CGovernanceVote& vote1, const CGovernanceVote& vote2)
{
    ...
    fResult = (vote1.vinSafenode == vote2.vinSafenode);

...
#if defined(NDEBUG)
# error "SafeNode cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "SafeNode Signed Message:\n";
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight < 500) {
        return 2 * COIN; // Anti instamine
    } 

	if (nPrevHeight <= 10000 && nPrevHeight >= 501) {
        return 20 * COIN;
	} 
		if (nPrevHeight <= 6500000 && nPrevHeight >= 10001) {
        return 15 * COIN;
	} 
		if (nPrevHeight <= 13000000 && nPrevHeight >= 650001) {
        return 10 * COIN;
	} 
...
CAmount GetSafenodePayment(int nHeight, CAmount blockValue)
{
...
void ThreadScriptCheck() {
    RenameThread("safenode-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // SXN : MODIFIED TO CHECK SAFENODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(SXN): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(SXN): couldn't find safenode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // SXN : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(SXN): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(SXN): spork is off, skipping transaction locking checks\n");
    }
...
bool ProcessNewBlock(CValidationState& state, const CChainParams& chainparams, const CNode* pfrom, const CBlock* pblock, bool fForceProcessing, CDiskBlockPos* dbp)
{
    ...
    safenodeSync.IsBlockchainSynced(true);
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /*
        SafeNode Related Inventory Messages
    ...
    */
    ...
    case MSG_SAFENODE_PAYMENT_VOTE:
        return mnpayments.mapSafenodePaymentVotes.count(inv.hash);

    case MSG_SAFENODE_PAYMENT_BLOCK:
        {
            BlockMap::iterator mi = mapBlockIndex.find(inv.hash);
            return mi != mapBlockIndex.end() && mnpayments.mapSafenodeBlocks.find(mi->second->nHeight) != mnpayments.mapSafenodeBlocks.end();
        }

    case MSG_SAFENODE_ANNOUNCE:
        return mnodeman.mapSeenSafenodeBroadcast.count(inv.hash) && !mnodeman.IsMnbRecoveryRequested(inv.hash);

    case MSG_SAFENODE_PING:
        return mnodeman.mapSeenSafenodePing.count(inv.hash);
    ...
    case MSG_SAFENODE_VERIFY:
        return mnodeman.mapSeenSafenodeVerification.count(inv.hash);
    }
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        LogPrint("net", "ProcessGetData -- inv = %s\n", inv.ToString());
        {
            ...
            else if (inv.IsKnownType())
            {
                ...
                if (!pushed && inv.type == MSG_SAFENODE_PAYMENT_VOTE) {
                    if(mnpayments.HasVerifiedPaymentVote(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnpayments.mapSafenodePaymentVotes[inv.hash];
                        pfrom->PushMessage(NetMsgType::SAFENODEPAYMENTVOTE, ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_SAFENODE_PAYMENT_BLOCK) {
                    BlockMap::iterator mi = mapBlockIndex.find(inv.hash);
                    LOCK(cs_mapSafenodeBlocks);
                    if (mi != mapBlockIndex.end() && mnpayments.mapSafenodeBlocks.count(mi->second->nHeight)) {
                        BOOST_FOREACH(CSafenodePayee& payee, mnpayments.mapSafenodeBlocks[mi->second->nHeight].vecPayees) {
                            std::vector<uint256> vecVoteHashes = payee.GetVoteHashes();
                            BOOST_FOREACH(uint256& hash, vecVoteHashes) {
                                if(mnpayments.HasVerifiedPaymentVote(hash)) {
                                    CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                                    ss.reserve(1000);
                                    ss << mnpayments.mapSafenodePaymentVotes[hash];
                                    pfrom->PushMessage(NetMsgType::SAFENODEPAYMENTVOTE, ss);
                                }
                            }
                        }
                ...
                if (!pushed && inv.type == MSG_SAFENODE_ANNOUNCE) {
                    if(mnodeman.mapSeenSafenodeBroadcast.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenSafenodeBroadcast[inv.hash].second;
                        pfrom->PushMessage(NetMsgType::MNANNOUNCE, ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_SAFENODE_PING) {
                    if(mnodeman.mapSeenSafenodePing.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenSafenodePing[inv.hash];
                ...
                if (!pushed && inv.type == MSG_SAFENODE_VERIFY) {
                    if(mnodeman.mapSeenSafenodeVerification.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenSafenodeVerification[inv.hash];
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    else if (strCommand == NetMsgType::TX || strCommand == NetMsgType::DSTX || strCommand == NetMsgType::TXLOCKREQUEST)
    {
        ...
        if (strCommand == NetMsgType::TXLOCKREQUEST) {
            ...
            CSafenode* pmn = mnodeman.Find(dstx.vin);
            if(pmn == NULL) {
                LogPrint("privatesend", "DSTX -- Can't find safenode %s to verify %s\n", dstx.vin.prevout.ToStringShort(), hashTx.ToString());
                return false;
            }

            if(!pmn->fAllowMixingTx) {
                LogPrint("privatesend", "DSTX -- Safenode %s is sending too many transactions %s\n", dstx.vin.prevout.ToStringShort(), hashTx.ToString());
                return true;
                // TODO: Not an error? Could it be that someone is relaying old DSTXes
                // we have no idea about (e.g we were offline)? How to handle them?
            }

            if(!dstx.CheckSignature(pmn->pubKeySafenode)) {
                LogPrint("privatesend", "DSTX -- CheckSignature() failed for %s\n", hashTx.ToString());
                return false;
            }

            LogPrintf("DSTX -- Got Safenode transaction %s\n", hashTx.ToString());
        ...
        if (!AlreadyHave(inv) && AcceptToMemoryPool(mempool, state, tx, true, &fMissingInputs))
        {
            ...
            if (strCommand == NetMsgType::DSTX) {
                LogPrintf("DSTX -- Safenode transaction accepted, txid=%s, peer=%d\n",
    ...
    else
    {
        ...
        if (found)
        {
            ...
            safenodeSync.ProcessMessage(pfrom, strCommand, vRecv);

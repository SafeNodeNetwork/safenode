...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total safenode balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized safenode balance of the wallet\n"
...
UniValue debug(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "safenode|privatesend|instantsend|safenode|spork|keepass|mnpayments|gobject )\n"
            "Change debug category on the fly. Specify single category or use comma to specify many.\n"
            "\nExamples:\n"
            + HelpExampleCli("debug", "safenode")
            + HelpExampleRpc("debug", "safenode,net")
        );
...
UniValue mnsync(const UniValue& params, bool fHelp)
{
    ...
    if(strMode == "status") {
        ...
        objStatus.push_back(Pair("AssetID", safenodeSync.GetAssetID()));
        objStatus.push_back(Pair("AssetName", safenodeSync.GetAssetName()));
        objStatus.push_back(Pair("Attempt", safenodeSync.GetAttempt()));
        objStatus.push_back(Pair("IsBlockchainSynced", safenodeSync.IsBlockchainSynced()));
        objStatus.push_back(Pair("IsSafenodeListSynced", safenodeSync.IsSafenodeListSynced()));
        objStatus.push_back(Pair("IsWinnersListSynced", safenodeSync.IsWinnersListSynced()));
        objStatus.push_back(Pair("IsSynced", safenodeSync.IsSynced()));
        objStatus.push_back(Pair("IsFailed", safenodeSync.IsFailed()));
        return objStatus;
    }

    if(strMode == "next")
    {
        safenodeSync.SwitchToNextAsset();
        return "sync updated to " + safenodeSync.GetAssetName();
    }
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"safenodeaddress\"\n"
            "\nReturn information about the given safenode address.\n"
            "\nArguments:\n"
            "1. \"safenodeaddress\"     (string, required) The safenode address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"safenodeaddress\", (string) The safenode address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: SafeNode address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are safenode addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) safenode address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"safenodeaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"safenodeaddress\"  (string, required) The safenode address to use for the signature.\n"

    ...
    platformStyle(platformStyle)
{
    ...
    QString windowTitle = tr("SafeNode") + " - ";
#ifdef ENABLE_WALLET
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a SafeNode address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and safenode: URIs)"));
...
#ifdef ENABLE_WALLET
    QSettings settings;
    if (settings.value("fShowSafenodesTab").toBool()) {
        safenodeAction = new QAction(QIcon(":/icons/" + theme + "/safenodes"), tr("&Safenodes"), this);
        safenodeAction->setStatusTip(tr("Browse safenodes"));
        safenodeAction->setToolTip(safenodeAction->statusTip());
        safenodeAction->setCheckable(true);
#ifdef Q_OS_MAC
        safenodeAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_5));
#else
        safenodeAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_5));
#endif
        tabGroup->addAction(safenodeAction);
        connect(safenodeAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
        connect(safenodeAction, SIGNAL(triggered()), this, SLOT(gotoSafenodePage()));
    }
    ...
    aboutAction = new QAction(QIcon(":/icons/" + theme + "/about"), tr("&About SafeNode"), this);
    aboutAction->setStatusTip(tr("Show information about SafeNode"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for SafeNode"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your SafeNode addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/" + theme + "/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified SafeNode addresses"));
    ...
    openMNConfEditorAction = new QAction(QIcon(":/icons/" + theme + "/edit"), tr("Open &Safenode Configuration File"), this);
    openMNConfEditorAction->setStatusTip(tr("Open Safenode configuration file"));    
    ...
    openAction->setStatusTip(tr("Open a safenode: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the SafeNode help message to get a list with possible SafeNode command-line options"));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        if (settings.value("fShowSafenodesTab").toBool())
        {
            toolbar->addAction(safenodeAction);
        }
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    QSettings settings;
    if (settings.value("fShowSafenodesTab").toBool() && safenodeAction) {
        safenodeAction->setEnabled(enabled);
    }
...
void BitcoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    ...
    QString toolTip = tr("SafeNode client") + " " + networkStyle->getTitleAddText();
...
void BitcoinGUI::gotoSafenodePage()
{
    QSettings settings;
    if (settings.value("fShowSafenodesTab").toBool()) {
        safenodeAction->setChecked(true);
        if (walletFrame) walletFrame->gotoSafenodePage();
    }
}
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to SafeNode network", "", count));
}
...
void BitcoinGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    ...
    if(safenodeSync.IsBlockchainSynced())
    {
        ...
        strSyncStatus = QString(safenodeSync.GetSyncStatus().c_str());
...
{
    QString strTitle = tr("SafeNode"); // default title

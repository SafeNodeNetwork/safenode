    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    QString version = tr("SafeNode") + " " + tr("version") + " " + QString::fromStdString(FormatFullVersion());
    ...
    if (helpMode == about)
    {
        setWindowTitle(tr("About SafeNode"));
        ...
        QString header = tr("Usage:") + "\n" +
            "  safenode-qt [" + tr("command-line options") + "]                     " + "\n";
...
All the SafeNode in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
PrivateSend uses an safenodetive process to mix your inputs with the inputs of two other people, without having your coins ever leave your wallet. \
...
These denominations are 0.01 SXN, 0.1 SXN, 1 SXN and 10 SXN -- sort of like the paper money you use every day.</li> \
<li>Your wallet then sends requests to specially configured software nodes on the network, called \"safenodes.\" \
These safenodes are informed then that you are interested in mixing a certain denomination. \
No identifiable information is sent to the safenodes, so they never know \"who\" you are.</li> \
...
The safenode mixes up the inputs and instructs all three users' wallets to pay the now-transformed input back to themselves. \
...
For more info see <a href=\"https://safenodecoin.atlassian.net/wiki/display/DOC/PrivateSend\">https://safenodecoin.atlassian.net/wiki/display/DOC/PrivateSend</a> \
        "));
        ui->aboutMessage->setWordWrap(true);
        ui->helpMessage->setVisible(false);
        ui->aboutLogo->setVisible(false);
    ...
    QWidget(parent, f)
{
    ...
    layout->addWidget(new QLabel(
        tr("SafeNode is shutting down...") + "<br /><br />" +

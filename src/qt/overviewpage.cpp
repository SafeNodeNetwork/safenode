...
public:
    TxViewDelegate(const PlatformStyle *platformStyle):
        QAbstractItemDelegate(), unit(BitcoinUnits::SXN),
    ...
    filter(0)
{
    ...
    // Disable any PS UI for safenode or when autobackup is disabled or failed for whatever reason
    if(fSafeNode || nWalletBackups <= 0){
...
{
    if(!fLiteMode && !fSafeNode) disconnect(timer, SIGNAL(timeout()), this, SLOT(privateSendStatus()));
    delete ui;
}
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    {
        // update the display unit, to not use the default ("SXN")
        updateDisplayUnit();
...
{
    if(!safenodeSync.IsBlockchainSynced() || ShutdownRequested()) return;

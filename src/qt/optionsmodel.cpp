...
void OptionsModel::Init(bool resetSettings)
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SXN);
    ...
    if (!settings.contains("fShowSafenodesTab"))
        settings.setValue("fShowSafenodesTab", safenodeConfig.getCount());
    ...
    if (!settings.contains("nPrivateSendAmount")) {
        ...
        if (!settings.contains("nAnonymizeSafeNodeAmount"))
            settings.setValue("nPrivateSendAmount", DEFAULT_PRIVATESEND_AMOUNT);
        else
            settings.setValue("nPrivateSendAmount", settings.value("nAnonymizeSafeNodeAmount").toInt());
    }
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in safenode.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case ShowSafenodesTab:
            return settings.value("fShowSafenodesTab");
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case SpendZeroConfChange:
            ...
            break;
        case ShowSafenodesTab:
            if (settings.value("fShowSafenodesTab") != value) {
                settings.setValue("fShowSafenodesTab", value);
                setRestartRequired(true);
            }
            break;
        case ShowAdvancedPSUI:

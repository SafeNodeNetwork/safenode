...
private:
    ...
    /// Safenode info for signed objects
    CTxIn vinSafenode;
    ...
    // VARIOUS FLAGS FOR OBJECT / SET VIA SAFENODE VOTING
...
public:
    ...
    const CTxIn& GetSafenodeVin() const {
        return vinSafenode;
    }
    ...
    void SetSafenodeInfo(const CTxIn& vin);
    bool Sign(CKey& keySafenode, CPubKey& pubKeySafenode);
    bool CheckSignature(CPubKey& pubKeySafenode);
    ...
    bool IsValidLocally(std::string& strError, bool& fMissingSafenode, bool fCheckCollateral);

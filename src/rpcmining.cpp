...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "  \"safenode\" : {                  (json object) required safenode payee that must be included in the next block\n"
            "      \"payee\" : \"xxxx\",             (string) payee address\n"
            "      \"script\" : \"xxxx\",            (string) payee scriptPubKey\n"
            "      \"amount\": n                   (numeric) required amount to pay\n"
            "  },\n"
            "  \"safenode_payments_started\" :  true|false, (boolean) true, if safenode payments started\n"
            "  \"safenode_payments_enforced\" : true|false, (boolean) true, if safenode payments are enforced\n"
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "SafeNode is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "SafeNode is downloading blocks...");

    if (!safenodeSync.IsSynced())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "SafeNode is syncing with network...");
    ...
    UniValue safenodeObj(UniValue::VOBJ);
    if(pblock->txoutSafenode != CTxOut()) {
        CTxDestination address1;
        ExtractDestination(pblock->txoutSafenode.scriptPubKey, address1);
        CBitcoinAddress address2(address1);
        safenodeObj.push_back(Pair("payee", address2.ToString().c_str()));
        safenodeObj.push_back(Pair("script", HexStr(pblock->txoutSafenode.scriptPubKey.begin(), pblock->txoutSafenode.scriptPubKey.end())));
        safenodeObj.push_back(Pair("amount", pblock->txoutSafenode.nValue));
    }
    result.push_back(Pair("safenode", safenodeObj));
    result.push_back(Pair("safenode_payments_started", pindexPrev->nHeight + 1 > Params().GetConsensus().nSafenodePaymentsStartBlock));
    result.push_back(Pair("safenode_payments_enforced", sporkManager.IsSporkActive(SPORK_8_SAFENODE_PAYMENT_ENFORCEMENT)));

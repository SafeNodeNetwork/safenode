...
// step 2) Top COutPointLock::SIGNATURES_TOTAL safenodes per each spent outpoint push "txvote" message
...
void CInstantSend::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    // Ignore any InstantSend messages until safenode list is synced
...
bool CInstantSend::ProcessTxLockRequest(const CTxLockRequest& txLockRequest)
{
    ...
    // Safenodes will sometimes propagate votes before the transaction is known to the client.
...
void CInstantSend::Vote(CTxLockCandidate& txLockCandidate)
{
    ...
    while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
        ...
        int n = mnodeman.GetSafenodeRank(activeSafenode.vin, nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);

        if(n == -1) {
            LogPrint("instantsend", "CInstantSend::Vote -- Unknown Safenode %s\n", activeSafenode.vin.prevout.ToStringShort());
            ++itOutpointLock;
            continue;
        }
        ...
        if(n > nSignaturesTotal) {
            LogPrint("instantsend", "CInstantSend::Vote -- Safenode not in the top %d (%d)\n", nSignaturesTotal, n);
            ++itOutpointLock;
            continue;
        }
        ...
        if(itVoted != mapVotedOutpoints.end()) {
            BOOST_FOREACH(const uint256& hash, itVoted->second) {
                ...
                if(it2->second.HasSafenodeVoted(itOutpointLock->first, activeSafenode.vin.prevout)) {
        ...
        CTxLockVote vote(txHash, itOutpointLock->first, activeSafenode.vin.prevout);
...
bool CInstantSend::ProcessTxLockVote(CNode* pfrom, CTxLockVote& vote)
{
    ...
    if(it == mapTxLockCandidates.end()) {
        if(!mapTxLockVotesOrphan.count(vote.GetHash())) {
            ...
            LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- Orphan vote: txid=%s  safenode=%s new\n",
                    txHash.ToString(), vote.GetSafenodeOutpoint().ToStringShort());
            ...
            LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- Orphan vote: txid=%s  safenode=%s seen\n",
                    txHash.ToString(), vote.GetSafenodeOutpoint().ToStringShort());
        }
        ...
        int nSafenodeOrphanExpireTime = GetTime() + 60*10; // keep time data for 10 minutes
        if(!mapSafenodeOrphanVotes.count(vote.GetSafenodeOutpoint())) {
            mapSafenodeOrphanVotes[vote.GetSafenodeOutpoint()] = nSafenodeOrphanExpireTime;
        } else {
            int64_t nPrevOrphanVote = mapSafenodeOrphanVotes[vote.GetSafenodeOutpoint()];
            if(nPrevOrphanVote > GetTime() && nPrevOrphanVote > GetAverageSafenodeOrphanVoteTime()) {
                LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- safenode is spamming orphan Transaction Lock Votes: txid=%s  safenode=%s\n",
                        txHash.ToString(), vote.GetSafenodeOutpoint().ToStringShort());
    ...
    if(it1 != mapVotedOutpoints.end()) {
        BOOST_FOREACH(const uint256& hash, it1->second) {
            if(hash != txHash) {
                ...
                std::map<uint256, CTxLockCandidate>::iterator it2 = mapTxLockCandidates.find(hash);
                if(it2->second.HasSafenodeVoted(vote.GetOutpoint(), vote.GetSafenodeOutpoint())) {
                    // yes, it did, refuse to accept a vote to include the same outpoint in another tx
                    // from the same safenode.
                    // TODO: apply pose ban score to this safenode?
                    ...
                    // pose ban score too.
                    LogPrintf("CInstantSend::ProcessTxLockVote -- safenode sent conflicting votes! %s\n", vote.GetSafenodeOutpoint().ToStringShort());
                    return false;
                }
            }
        }
        // we have votes by other safenodes only (so far), let's continue and see who will win
...
int64_t CInstantSend::GetAverageSafenodeOrphanVoteTime()
{
    LOCK(cs_instantsend);
    // NOTE: should never actually call this function when mapSafenodeOrphanVotes is empty
    if(mapSafenodeOrphanVotes.empty()) return 0;

    std::map<COutPoint, int64_t>::iterator it = mapSafenodeOrphanVotes.begin();
    int64_t total = 0;

    while(it != mapSafenodeOrphanVotes.end()) {
        total+= it->second;
        ++it;
    }

    return total / mapSafenodeOrphanVotes.size();
}
...
void CInstantSend::CheckAndRemove()
{
    ...
    while(itVote != mapTxLockVotes.end()) {
        if(itVote->second.IsExpired(pCurrentBlockIndex->nHeight)) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired vote: txid=%s  safenode=%s\n",
                    itVote->second.GetTxHash().ToString(), itVote->second.GetSafenodeOutpoint().ToStringShort());
    ...
    while(itOrphanVote != mapTxLockVotesOrphan.end()) {
        if(GetTime() - itOrphanVote->second.GetTimeCreated() > ORPHAN_VOTE_SECONDS) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan vote: txid=%s  safenode=%s\n",
                    itOrphanVote->second.GetTxHash().ToString(), itOrphanVote->second.GetSafenodeOutpoint().ToStringShort());
    ...
    // remove expired safenode orphan votes (DOS protection)
    std::map<COutPoint, int64_t>::iterator itSafenodeOrphan = mapSafenodeOrphanVotes.begin();
    while(itSafenodeOrphan != mapSafenodeOrphanVotes.end()) {
        if(itSafenodeOrphan->second < GetTime()) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan safenode vote: safenode=%s\n",
                    itSafenodeOrphan->first.ToStringShort());
            mapSafenodeOrphanVotes.erase(itSafenodeOrphan++);
        } else {
            ++itSafenodeOrphan;
        }
    }
...
{
    if(!mnodeman.Has(CTxIn(outpointSafenode))) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Unknown safenode %s\n", outpointSafenode.ToStringShort());
        mnodeman.AskForMN(pnode, CTxIn(outpointSafenode));
        return false;
    }
    ...
    int n = mnodeman.GetSafenodeRank(CTxIn(outpointSafenode), nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);

    if(n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("instantsend", "CTxLockVote::IsValid -- Outdated safenode %s\n", outpointSafenode.ToStringShort());
        return false;
    }
    LogPrint("instantsend", "CTxLockVote::IsValid -- Safenode %s, rank=%d\n", outpointSafenode.ToStringShort(), n);

    int nSignaturesTotal = COutPointLock::SIGNATURES_TOTAL;
    if(n > nSignaturesTotal) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Safenode %s is not in the top %d (%d), vote hash=%s\n",
                outpointSafenode.ToStringShort(), nSignaturesTotal, n, GetHash().ToString());
        return false;
    }
...
bool CTxLockVote::CheckSignature() const
{
    ...
    safenode_info_t infoMn = mnodeman.GetSafenodeInfo(CTxIn(outpointSafenode));

    if(!infoMn.fInfoValid) {
        LogPrintf("CTxLockVote::CheckSignature -- Unknown Safenode: safenode=%s\n", outpointSafenode.ToString());
        return false;
    }

    if(!darkSendSigner.VerifyMessage(infoMn.pubKeySafenode, vchSafenodeSignature, strMessage, strError)) {
...
bool CTxLockVote::Sign()
{
    ...
    if(!darkSendSigner.SignMessage(strMessage, vchSafenodeSignature, activeSafenode.keySafenode)) {
        LogPrintf("CTxLockVote::Sign -- SignMessage() failed\n");
        return false;
    }

    if(!darkSendSigner.VerifyMessage(activeSafenode.pubKeySafenode, vchSafenodeSignature, strMessage, strError)) {
...
{
    if(mapSafenodeVotes.count(vote.GetSafenodeOutpoint()))
        return false;
    mapSafenodeVotes.insert(std::make_pair(vote.GetSafenodeOutpoint(), vote));
    return true;
}
...
std::vector<CTxLockVote> COutPointLock::GetVotes() const
{
    ...
    std::map<COutPoint, CTxLockVote>::const_iterator itVote = mapSafenodeVotes.begin();
    while(itVote != mapSafenodeVotes.end()) {
...
bool COutPointLock::HasSafenodeVoted(const COutPoint& outpointSafenodeIn) const
{
    return mapSafenodeVotes.count(outpointSafenodeIn);
}
...
bool CTxLockCandidate::HasSafenodeVoted(const COutPoint& outpointIn, const COutPoint& outpointSafenodeIn)
{
    std::map<COutPoint, COutPointLock>::iterator it = mapOutPointLocks.find(outpointIn);
    return it !=mapOutPointLocks.end() && it->second.HasSafenodeVoted(outpointSafenodeIn);
}

...
//SafeNode only features
bool fSafeNode = false;
...
const char * const BITCOIN_CONF_FILENAME = "wallet.conf";
const char * const BITCOIN_PID_FILENAME = "safenoded.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "safenode" is a composite category enabling all SafeNode-related debug output
            if(ptrCategory->count(string("safenode"))) {
                ptrCategory->insert(string("privatesend"));
                ptrCategory->insert(string("instantsend"));
                ptrCategory->insert(string("safenode"));
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SafeNode
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SafeNode
    // Mac: ~/Library/Application Support/SafeNode
    // Unix: ~/.safenode
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SafeNode";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SafeNode";
#else
    // Unix
    return pathRet / ".safenode";
#endif
#endif
}
...
boost::filesystem::path GetSafenodeConfigFile()
{
    boost::filesystem::path pathConfigFile(GetArg("-mnconf", "safenode.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty safenode.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override safenode.conf

...
void CGovernanceManager::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
...

    {
        ...
        if(!safenodeSync.IsSafenodeListSynced()) {
            LogPrint("gobject", "MNGOVERNANCEOBJECT -- safenode list not synced\n");
            return;
        }
        ...
        if(!SafenodeRateCheck(govobj, UPDATE_FAIL_ONLY, false, fRateCheckBypassed)) {
            LogPrintf("MNGOVERNANCEOBJECT -- safenode rate check failed - %s - (current block height %d) \n", strHash, nCachedBlockHeight);
            return;
        }
        ...
        bool fSafenodeMissing = false;
        bool fIsValid = govobj.IsValidLocally(strError, fSafenodeMissing, true);

        if(fSafenodeMissing) {
            mapSafenodeOrphanObjects.insert(std::make_pair(nHash, object_time_pair_t(govobj, GetAdjustedTime() + GOVERNANCE_ORPHAN_EXPIRATION_TIME)));
            LogPrintf("MNGOVERNANCEOBJECT -- Missing safenode for: %s, strError = %s\n", strHash, strError);
        ...
        if(fRateCheckBypassed) {
            if(!SafenodeRateCheck(govobj, UPDATE_FAIL_ONLY, true, fRateCheckBypassed)) {
                LogPrintf("MNGOVERNANCEOBJECT -- safenode rate check failed (after signature verification) - %s - (current block height %d) \n", strHash, nCachedBlockHeight);
                return;
            }
        }
        ...
        if(fAddToSeen) {
            ...
            SafenodeRateCheck(govobj, UPDATE_TRUE, true, fRateCheckBypassed);
        }

        safenodeSync.AddedGovernanceItem();
    ...
    {
        // Ignore such messages until safenode list is synced
        if(!safenodeSync.IsSafenodeListSynced()) {
            LogPrint("gobject", "MNGOVERNANCEOBJECTVOTE -- safenode list not synced\n");
            return;
        }
        ...
        if(ProcessVote(pfrom, vote, exception)) {
            ...
            if((exception.GetNodePenalty() != 0) && safenodeSync.IsSynced()) {
...
std::vector<CGovernanceVote> CGovernanceManager::GetCurrentVotes(const uint256& nParentHash, const CTxIn& mnCollateralOutpointFilter)
{
    ...
    // Compile a list of Safenode collateral outpoints for which to get votes
    std::vector<CTxIn> vecMNTxIn;
    if (mnCollateralOutpointFilter == CTxIn()) {
        std::vector<CSafenode> mnlist = mnodeman.GetFullSafenodeVector();
        for (std::vector<CSafenode>::iterator it = mnlist.begin(); it != mnlist.end(); ++it)
        {
...
void CGovernanceManager::Sync(CNode* pfrom, const uint256& nProp, const CBloomFilter& filter)
{
    ...
    */
    ...
    if(fSafeNode && !safenodeSync.IsSynced()) return;
    ...
    pfrom->PushMessage(NetMsgType::SYNCSTATUSCOUNT, SAFENODE_SYNC_GOVOBJ, nObjCount);
    pfrom->PushMessage(NetMsgType::SYNCSTATUSCOUNT, SAFENODE_SYNC_GOVOBJ_VOTE, nVoteCount);
    LogPrintf("CGovernanceManager::Sync -- sent %d objects and %d votes to peer=%d\n", nObjCount, nVoteCount, pfrom->id);
}

bool CGovernanceManager::SafenodeRateCheck(const CGovernanceObject& govobj, update_mode_enum_t eUpdateLast)
{
    bool fRateCheckBypassed = false;
    return SafenodeRateCheck(govobj, eUpdateLast, true, fRateCheckBypassed);
}

bool CGovernanceManager::SafenodeRateCheck(const CGovernanceObject& govobj, update_mode_enum_t eUpdateLast, bool fForce, bool& fRateCheckBypassed)
{
    LOCK(cs);
    ...
    const CTxIn& vin = govobj.GetSafenodeVin();

    txout_m_it it  = mapLastSafenodeObject.find(vin.prevout);

    if(it == mapLastSafenodeObject.end()) {
        if(eUpdateLast == UPDATE_TRUE) {
            it = mapLastSafenodeObject.insert(txout_m_t::value_type(vin.prevout, last_object_rec(true))).first;
            switch(nObjectType) {
            case GOVERNANCE_OBJECT_TRIGGER:
    ...
    if(nTimestamp < nNow - 2 * nSuperblockCycleSeconds) {
        LogPrintf("CGovernanceManager::SafenodeRateCheck -- object %s rejected due to too old timestamp, safenode vin = %s, timestamp = %d, current time = %d\n",
                 strHash, vin.prevout.ToStringShort(), nTimestamp, nNow);
        return false;
    }

    if(nTimestamp > nNow + 60*60) {
        LogPrintf("CGovernanceManager::SafenodeRateCheck -- object %s rejected due to too new (future) timestamp, safenode vin = %s, timestamp = %d, current time = %d\n",
                 strHash, vin.prevout.ToStringShort(), nTimestamp, nNow);
        return false;
    }
    ...
    if(!pBuffer) {
        LogPrintf("CGovernanceManager::SafenodeRateCheck -- Internal Error returning false, NULL ptr found for object %s safenode vin = %s, timestamp = %d, current time = %d\n",
                  strHash, vin.prevout.ToStringShort(), nTimestamp, nNow);
        return false;
    }
    ...
    if(fRateOK) {
        ...
        LogPrintf("CGovernanceManager::SafenodeRateCheck -- Rate too high: object hash = %s, safenode vin = %s, object timestamp = %d, rate = %f, max rate = %f\n",
...
bool CGovernanceManager::ProcessVote(CNode* pfrom, const CGovernanceVote& vote, CGovernanceException& exception)
{
    ...
    if(mapInvalidVotes.HasKey(nHashVote)) {
        ...
        ostr << "CGovernanceManager::ProcessVote -- Old invalid vote "
                << ", MN outpoint = " << vote.GetVinSafenode().prevout.ToStringShort()
    ...
    if(fOk) {
        ...
        if(govobj.GetObjectType() == GOVERNANCE_OBJECT_WATCHDOG) {
            mnodeman.UpdateWatchdogVoteTime(vote.GetVinSafenode());
        }
    }
    return fOk;
}

void CGovernanceManager::CheckSafenodeOrphanVotes()
{
    LOCK2(cs_main, cs);
    fRateChecksEnabled = false;
    for(object_m_it it = mapObjects.begin(); it != mapObjects.end(); ++it) {
...
void CGovernanceManager::CheckSafenodeOrphanObjects()
{
    LOCK2(cs_main, cs);
    int64_t nNow = GetAdjustedTime();
    fRateChecksEnabled = false;
    object_time_m_it it = mapSafenodeOrphanObjects.begin();
    while(it != mapSafenodeOrphanObjects.end()) {
        object_time_pair_t& pair = it->second;
        CGovernanceObject& govobj = pair.first;

        if(pair.second < nNow) {
            mapSafenodeOrphanObjects.erase(it++);
            continue;
        }
        ...
        if(AddGovernanceObject(govobj, fAddToSeen)) {
            LogPrintf("CGovernanceManager::CheckSafenodeOrphanObjects -- %s new\n", govobj.GetHash().ToString());
            govobj.Relay();
            mapSafenodeOrphanObjects.erase(it++);
        }
        else {
...
int CGovernanceManager::RequestGovernanceObjectVotes(const std::vector<CNode*>& vNodesCopy)
{
    ...
    // Testnet is ~40 times smaller in safenode count, but only ~1000 safenodes usually vote,
    // so 1 obj on mainnet == ~10 objs or ~1000 votes on testnet. However we want to test a higher
    // number of votes to make sure it's robust enough, so aim at 2000 votes per safenode per request.
    ...
    for (int i = 0; i < nMaxObjRequestsPerNode; ++i) {
        ...
        BOOST_FOREACH(CNode* pnode, vNodesCopy) {
            // Only use reqular peers, don't try to ask from outbound "safenode" connections -
            // they stay connected for a short period of time and it's possible that we won't get everything we should.
            // Only use outbound connections - inbound connection could be a "safenode" connection
...
int CGovernanceManager::GetSafenodeIndex(const CTxIn& safenodeVin)
{
    LOCK(cs);
    bool fIndexRebuilt = false;
    int nMNIndex = mnodeman.GetSafenodeIndex(safenodeVin, fIndexRebuilt);
    if(fIndexRebuilt) {
        RebuildVoteMaps();
        nMNIndex = mnodeman.GetSafenodeIndex(safenodeVin, fIndexRebuilt);
        if(fIndexRebuilt) {
            LogPrintf("CGovernanceManager::GetSafenodeIndex -- WARNING: vote map rebuild failed\n");
        }
    }
    return nMNIndex;
}
...
void CGovernanceManager::RebuildVoteMaps()
{
    ...
    mnodeman.ClearOldSafenodeIndex();
}
...
void CGovernanceManager::InitOnLoad()
{
    ...
    LogPrintf("Preparing safenode indexes and governance triggers...\n");
    RebuildIndexes();
    AddCachedTriggers();
    LogPrintf("Safenode indexes and governance triggers prepared  %dms\n", GetTimeMillis() - nStart);
    LogPrintf("     %s\n", ToString());
}

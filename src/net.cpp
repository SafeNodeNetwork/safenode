...
namespace {
    ...
    const int MAX_OUTBOUND_SAFENODE_CONNECTIONS = 100;
...
static CSemaphore *semSafenodeOutbound = NULL;
...
CNode* ConnectNode(CAddress addrConnect, const char *pszDest, bool fConnectToSafenode)
{
    if (pszDest == NULL) {
        // we clean safenode connections in CSafenodeMan::ProcessSafenodeConnections()
        // so should be safe to skip this and connect to local Hot MN on CActiveSafenode::ManageState()
        if (IsLocal(addrConnect) && !fConnectToSafenode)
            return NULL;
        ...
        if (pnode)
        {
            // we have existing connection to this node but it was not a connection to safenode,
            // change flag and add reference so that we can correctly clear it later
            if(fConnectToSafenode && !pnode->fSafenode) {
                pnode->AddRef();
                pnode->fSafenode = true;
            }
            return pnode;
        }
    }
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (SafeNode users should be well used to small numbers with many decimal places by now :)
...
static void AcceptConnection(const ListenSocket& hListenSocket) {
    ...
    if(fSafeNode && !safenodeSync.IsSynced()) {
        LogPrintf("AcceptConnection -- safenode is not synced yet, skipping inbound connection attempt\n");
        CloseSocket(hSocket);
        return;
    }
    ...
    while (true)
    {
        ...
        //
        {
            ...
            BOOST_FOREACH(CNode* pnode, vNodesCopy)
            {
                ...
                {
                    LogPrintf("ThreadSocketHandler -- removing node: peer=%d addr=%s nRefCount=%d fNetworkNode=%d fInbound=%d fSafenode=%d\n",
                              pnode->id, pnode->addr.ToString(), pnode->GetRefCount(), pnode->fNetworkNode, pnode->fInbound, pnode->fSafenode);
                    ...
                    pnode->grantSafenodeOutbound.Release();
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "SafeNode " + FormatFullVersion();
...
{
    // Connecting to specific addresses, no safenode connections available
    if (mapArgs.count("-connect") && mapMultiArgs["-connect"].size() > 0)
        return;
    ...
    while (true)
    {
        ...
        CSemaphoreGrant grant(*semSafenodeOutbound);
        boost::this_thread::interruption_point();
        ...
        grant.MoveTo(pnode->grantSafenodeOutbound);
        ...
        while(it != p.second.end()) {
            if(*it != uint256()) {
                vToFetch.push_back(CInv(MSG_SAFENODE_ANNOUNCE, *it));
                LogPrint("safenode", "ThreadMnbRequestConnections -- asking for mnb %s from addr=%s\n", it->ToString(), p.first.ToString());
            }
            ++it;
        }
...
bool BindListenPort(const CService &addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. SafeNode is probably already running."), addrBind.ToString());
        else
...
void StartNode(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    if (semSafenodeOutbound == NULL) {
        // initialize semaphore
        semSafenodeOutbound = new CSemaphore(MAX_OUTBOUND_SAFENODE_CONNECTIONS);
    }
...
bool StopNode()
{
    ...
    if (semSafenodeOutbound)
        for (int i=0; i<MAX_OUTBOUND_SAFENODE_CONNECTIONS; i++)
            semSafenodeOutbound->post();
...
public:
    ...
    ~CNetCleanup()
    {
        ...
        delete semOutbound;
        semOutbound = NULL;
        delete semSafenodeOutbound;
        semSafenodeOutbound = NULL;

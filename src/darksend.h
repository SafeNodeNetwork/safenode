...
// A helper object for signing messages from Safenodes
...
public:
     ...
     *     1) we have an active Safenode,
     *     2) we have a valid Safenode private key,
     *     3) we signed the message successfully, and
     *     4) we verified the message successfully
     */
    bool Sign();
    /// Check if we have a valid Safenode address
    bool CheckSignature(const CPubKey& pubKeySafenode);
    ...
    std::string ToString()
    {
        return strprintf("nDenom=%d, nTime=%lld, fReady=%s, fTried=%s, safenode=%s",
...
{
public:
    /// Is the input associated with this public key? (and there is 2500 SXN - checking if valid safenode)
...
private:
    ...
    // Keep track of the used Safenodes
    std::vector<CTxIn> vecSafenodesUsed;
    ...
    std::vector<CDarkSendEntry> vecEntries; // Safenode/clients entries
    ...
    /// Get the denominations for a specific amount of safenode.
    ...
    /// As a client, submit part of a future mixing transaction to a Safenode to start the process
    ...
    /// Get Safenode updates about the progress of mixing
...
public:
    CSafenode* pSubmittedToSafenode;

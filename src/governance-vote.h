...
// INTENTION OF SAFENODES REGARDING ITEM
...
//
// CGovernanceVote - Allow a safenode node to vote and broadcast throughout the network
//
...
public:
    CGovernanceVote();
    CGovernanceVote(CTxIn vinSafenodeIn, uint256 nParentHashIn, vote_signal_enum_t eVoteSignalIn, vote_outcome_enum_t eVoteOutcomeIn);
    ...
    CTxIn& GetVinSafenode() { return vinSafenode; }

    const CTxIn& GetVinSafenode() const { return vinSafenode; }
    ...
    *   GetTypeHash()
    *
    *   GET HASH WITH DETERMINISTIC VALUE OF SAFENODE-VIN/PARENT-HASH/VOTE-SIGNAL
    *
    *   This hash collides with previous safenode votes when they update their votes on governance objects.
    ...
    *   --
    *
    *   We do not include an outcome, because that can change when a safenode updates their vote from yes to no

...
{
    if(fLiteMode) return; // ignore all SafeNode related functionality
    ...
    if(strCommand == NetMsgType::DSACCEPT) {
        ...
        if(!fSafeNode) {
            LogPrintf("DSACCEPT -- not a Safenode!\n");
        ...
        CSafenode* pmn = mnodeman.Find(activeSafenode.vin);
        if(pmn == NULL) {
        ...
        CSafenode* pmn = mnodeman.Find(dsq.vin);
        if(pmn == NULL) return;

        if(!dsq.CheckSignature(pmn->pubKeySafenode)) {
        ...
        if(dsq.fReady) {
            if(!pSubmittedToSafenode) return;
            if((CNetAddr)pSubmittedToSafenode->addr != (CNetAddr)pmn->addr) {
                LogPrintf("DSQUEUE -- message doesn't match current Safenode: pSubmittedToSafenode=%s, addr=%s\n", pSubmittedToSafenode->addr.ToString(), pmn->addr.ToString());
                return;
            }

            if(nState == POOL_STATE_QUEUE) {
                LogPrint("privatesend", "DSQUEUE -- PrivateSend queue (%s) is ready on safenode %s\n", dsq.ToString(), pmn->addr.ToString());
                SubmitDenominate();
            }
        } else {
            BOOST_FOREACH(CDarksendQueue q, vecDarksendQueue) {
                if(q.vin == dsq.vin) {
                    ...
                    LogPrint("privatesend", "DSQUEUE -- Safenode %s is sending WAY too many dsq messages\n", pmn->addr.ToString());
                    return;
                }
            }
            ...
            if(pmn->nLastDsq != 0 && nThreshold > mnodeman.nDsqCount) {
                LogPrint("privatesend", "DSQUEUE -- Safenode %s is sending too many dsq messages\n", pmn->addr.ToString());
                return;
            }
            mnodeman.nDsqCount++;
            ...
            LogPrint("privatesend", "DSQUEUE -- new PrivateSend queue (%s) from safenode %s\n", dsq.ToString(), pmn->addr.ToString());
            if(pSubmittedToSafenode && pSubmittedToSafenode->vin.prevout == dsq.vin.prevout) {
                dsq.fTried = true;
            }
        ...
        if(!fSafeNode) {
            LogPrintf("DSVIN -- not a Safenode!\n");
            PushStatus(pfrom, STATUS_REJECTED, ERR_NOT_A_MN);
            return;
        }
        ...
        if(fSafeNode) {
            // LogPrintf("DSSTATUSUPDATE -- Can't run on a Safenode!\n");
            return;
        }

        if(!pSubmittedToSafenode) return;
        if((CNetAddr)pSubmittedToSafenode->addr != (CNetAddr)pfrom->addr) {
            //LogPrintf("DSSTATUSUPDATE -- message doesn't match current Safenode: pSubmittedToSafenode %s addr %s\n", pSubmittedToSafenode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if(!fSafeNode) {
            LogPrintf("DSSIGNFINALTX -- not a Safenode!\n");
            return;
        }
        ...
        if(fSafeNode) {
            // LogPrintf("DSFINALTX -- Can't run on a Safenode!\n");
            return;
        }

        if(!pSubmittedToSafenode) return;
        if((CNetAddr)pSubmittedToSafenode->addr != (CNetAddr)pfrom->addr) {
            //LogPrintf("DSFINALTX -- message doesn't match current Safenode: pSubmittedToSafenode %s addr %s\n", pSubmittedToSafenode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if(fSafeNode) {
            // LogPrintf("DSCOMPLETE -- Can't run on a Safenode!\n");
            return;
        }

        if(!pSubmittedToSafenode) return;
        if((CNetAddr)pSubmittedToSafenode->addr != (CNetAddr)pfrom->addr) {
            LogPrint("privatesend", "DSCOMPLETE -- message doesn't match current Safenode: pSubmittedToSafenode=%s  addr=%s\n", pSubmittedToSafenode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
...
std::string CDarksendPool::GetStatus()
{
    ...
    if((pCurrentBlockIndex && pCurrentBlockIndex->nHeight - nCachedLastSuccessBlock < nMinBlockSpacing) || !safenodeSync.IsBlockchainSynced())
    ...
    switch(nState) {
        ...
        case POOL_STATE_QUEUE:
            ...
            return strprintf(_("Submitted to safenode, waiting in queue %s"), strSuffix);;
        case POOL_STATE_ACCEPTING_ENTRIES:
            if(nEntriesCount == 0) {
                ...
                if(     nStatusMessageProgress % 70 <= 40) return strprintf(_("Submitted following entries to safenode: %u / %d"), nEntriesCount, GetMaxPoolTransactions());
                else if(nStatusMessageProgress % 70 <= 50) strSuffix = ".";
                else if(nStatusMessageProgress % 70 <= 60) strSuffix = "..";
                else if(nStatusMessageProgress % 70 <= 70) strSuffix = "...";
                return strprintf(_("Submitted to safenode, waiting for more entries ( %u / %d ) %s"), nEntriesCount, GetMaxPoolTransactions(), strSuffix);
            }
...
//
// Check the mixing progress and send client updates if a Safenode
//
...
{
    if(!fSafeNode) return; // check and relay final tx only on safenode
    ...
    // create and sign safenode dstx transaction
    if(!mapDarksendBroadcastTxes.count(hashTx)) {
        CDarksendBroadcastTx dstx(finalTransaction, activeSafenode.vin, GetAdjustedTime());
        dstx.Sign();
...
//
// How does this work? Messages to Safenodes come in via NetMsgType::DSVIN, these require a valid collateral
// transaction for the client to be able to enter the pool. This transaction is kept by the Safenode
...
/*
    ...
    allow endless transaction that would bloat SafeNode and make it unusable. To
...
void CDarksendPool::CheckTimeout()
{
    ...
    if(!fEnablePrivateSend && !fSafeNode) return;
    ...
    int nLagTime = fSafeNode ? 0 : 10000; // if we're the client, give the server a few extra seconds before resetting.
...
void CDarksendPool::CheckForCompleteQueue()
{
    ...
    if(nState == POOL_STATE_QUEUE && IsSessionReady()) {
        ...
        CDarksendQueue dsq(nSessionDenom, activeSafenode.vin, GetTime(), true);
...
//
// Execute a mixing denomination via a Safenode.
// This is only ran from clients
//
bool CDarksendPool::SendDenominate(const std::vector<CTxIn>& vecTxIn, const std::vector<CTxOut>& vecTxOut)
{
    if(fSafeNode) {
        LogPrintf("CDarksendPool::SendDenominate -- PrivateSend from a Safenode is not supported currently.\n");
        return false;
    }
    ...
    // we should already be connected to a Safenode
    if(!nSessionID) {
        LogPrintf("CDarksendPool::SendDenominate -- No Safenode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return false;
    }
...
// Incoming message from Safenode updating the progress of mixing
bool CDarksendPool::CheckPoolStateUpdate(PoolState nStateNew, int nEntriesCountNew, PoolStatusUpdate nStatusUpdate, PoolMessage nMessageID, int nSessionIDNew)
{
    ...
    strAutoDenomResult = _("Safenode:") + " " + GetMessageByID(nMessageID);

    // if rejected at any state
    if(nStatusUpdate == STATUS_REJECTED) {
        LogPrintf("CDarksendPool::CheckPoolStateUpdate -- entry is rejected by Safenode\n");
        UnlockCoins();
        SetNull();
        SetState(POOL_STATE_ERROR);
...
bool CDarksendPool::SignFinalTransaction(const CTransaction& finalTransactionNew, CNode* pnode)
{
    ...
    // push all of our signatures to the Safenode
    LogPrintf("CDarksendPool::SignFinalTransaction -- pushing sigs to the safenode, finalMutableTransaction=%s", finalMutableTransaction.ToString());
...
{
    if(!fEnablePrivateSend || fSafeNode || !pCurrentBlockIndex) return false;
    ...
    if(mnodeman.size() == 0) {
        LogPrint("privatesend", "CDarksendPool::DoAutomaticDenominating -- No Safenodes detected\n");
        strAutoDenomResult = _("No Safenodes detected.");
        return false;
    }
    ...
    // If we've used 90% of the Safenode list then drop the oldest first ~30%
    int nThreshold_high = nMnCountEnabled * 0.9;
    int nThreshold_low = nThreshold_high * 0.7;
    LogPrint("privatesend", "Checking vecSafenodesUsed: size: %d, threshold: %d\n", (int)vecSafenodesUsed.size(), nThreshold_high);

    if((int)vecSafenodesUsed.size() > nThreshold_high) {
        vecSafenodesUsed.erase(vecSafenodesUsed.begin(), vecSafenodesUsed.begin() + vecSafenodesUsed.size() - nThreshold_low);
        LogPrint("privatesend", "  vecSafenodesUsed: new size: %d, threshold: %d\n", (int)vecSafenodesUsed.size(), nThreshold_high);
    }
    ...
    if(nLiquidityProvider || fUseQueue) {
        ...
        BOOST_FOREACH(CDarksendQueue& dsq, vecDarksendQueue) {
            ...
            CSafenode* pmn = mnodeman.Find(dsq.vin);
            if(pmn == NULL) {
                LogPrintf("CDarksendPool::DoAutomaticDenominating -- dsq safenode is not in safenode list, safenode=%s\n", dsq.vin.prevout.ToStringShort());
                continue;
            }
            ...
            vecSafenodesUsed.push_back(dsq.vin);
            ...
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- attempt to connect to safenode from queue, addr=%s\n", pmn->addr.ToString());
            // connect to Safenode and submit the queue request
            CNode* pnode = (pnodeFound && pnodeFound->fSafenode) ? pnodeFound : ConnectNode((CAddress)pmn->addr, NULL, true);
            if(pnode) {
                pSubmittedToSafenode = pmn;
                ...
                strAutoDenomResult = _("Error connecting to Safenode.");
                continue;
            }
        }
    }
    ...
    while(nTries < 10) {
        CSafenode* pmn = mnodeman.FindRandomNotInVec(vecSafenodesUsed, MIN_PRIVATESEND_PEER_PROTO_VERSION);
        if(pmn == NULL) {
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- Can't find random safenode!\n");
            strAutoDenomResult = _("Can't find random Safenode.");
            return false;
        }
        vecSafenodesUsed.push_back(pmn->vin);

        if(pmn->nLastDsq != 0 && pmn->nLastDsq + nMnCountEnabled/5 > mnodeman.nDsqCount) {
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- Too early to mix on this safenode!"
                        " safenode=%s  addr=%s  nLastDsq=%d  CountEnabled/5=%d  nDsqCount=%d\n",
        ...
        LogPrintf("CDarksendPool::DoAutomaticDenominating -- attempt %d connection to Safenode %s\n", nTries, pmn->addr.ToString());
        CNode* pnode = (pnodeFound && pnodeFound->fSafenode) ? pnodeFound : ConnectNode((CAddress)pmn->addr, NULL, true);
        if(pnode) {
    ...
    strAutoDenomResult = _("No compatible Safenode found.");
    return false;
}
...
{
    if(!fSafeNode || nSessionID != 0) return false;
    ...
    if(!fUnitTest) {
        ...
        CDarksendQueue dsq(nDenom, activeSafenode.vin, GetTime(), false);
...
{
    if(!fSafeNode || nSessionID == 0 || IsSessionReady()) return false;
...
std::string CDarksendPool::GetMessageByID(PoolMessage nMessageID)
{
    switch (nMessageID) {
        ...
        case ERR_MN_LIST:               return _("Not in the Safenode list.");
        case ERR_MODE:                  return _("Incompatible mode.");
        case ERR_NON_STANDARD_PUBKEY:   return _("Non-standard public key detected.");
        case ERR_NOT_A_MN:              return _("This is not a Safenode.");
        case ERR_QUEUE_FULL:            return _("Safenode queue is full.");
...
bool CDarksendQueue::Sign()
{
    ...
    return CheckSignature(activeSafenode.pubKeySafenode);
}

bool CDarksendQueue::CheckSignature(const CPubKey& pubKeySafenode)
{
    std::string strMessage = vin.ToString() + boost::lexical_cast<std::string>(nDenom) + boost::lexical_cast<std::string>(nTime) + boost::lexical_cast<std::string>(fReady);
    std::string strError = "";

    if(!darkSendSigner.VerifyMessage(pubKeySafenode, vchSig, strMessage, strError)) {
        LogPrintf("CDarksendQueue::CheckSignature -- Got bad Safenode queue signature: %s; error: %s\n", ToString(), strError);
        return false;
    }
...
bool CDarksendBroadcastTx::CheckSignature(const CPubKey& pubKeySafenode)
{
    std::string strMessage = tx.GetHash().ToString() + boost::lexical_cast<std::string>(sigTime);
    std::string strError = "";
...
void CDarksendPool::RelayIn(const CDarkSendEntry& entry)
{
    ...
    CNode* pnode = FindNode(pSubmittedToSafenode->addr);
    if(pnode != NULL) {
...
{
    if(fSafeNode && (nStateNew == POOL_STATE_ERROR || nStateNew == POOL_STATE_SUCCESS)) {
        LogPrint("privatesend", "CDarksendPool::SetState -- Can't set state to ERROR or SUCCESS as a Safenode. \n");
        return;
    }
...
void CDarksendPool::UpdatedBlockTip(const CBlockIndex *pindex)
{
    ...
    if(!fLiteMode && safenodeSync.IsSafenodeListSynced()) {
        NewBlock();
    }
}
...
void ThreadCheckDarkSendPool()
{
    ...
    RenameThread("safenode-privatesend");
    ...
    while (true)
    {
        ...
        safenodeSync.ProcessTick();

        if(safenodeSync.IsBlockchainSynced() && !ShutdownRequested()) {

            nTick++;

            // make sure to check all safenodes first
            mnodeman.Check();

            // check if we should activate or ping every few minutes,
            // slightly postpone first run to give net thread a chance to connect to some peers
            if(nTick % SAFENODE_MIN_MNP_SECONDS == 15)
                activeSafenode.ManageState();

            if(nTick % 60 == 0) {
                mnodeman.ProcessSafenodeConnections();
                mnodeman.CheckAndRemove();
                mnpayments.CheckAndRemove();
                instantsend.CheckAndRemove();
            }
            if(fSafeNode && (nTick % (60 * 5) == 0)) {

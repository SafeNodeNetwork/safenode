...
private:
    ...
    object_time_m_t mapSafenodeOrphanObjects;
    ...
    txout_m_t mapLastSafenodeObject;
...
public:
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
        ...
        READWRITE(mapLastSafenodeObject);
    ...
    bool SafenodeRateCheck(const CGovernanceObject& govobj, update_mode_enum_t eUpdateLast = UPDATE_FALSE);

    bool SafenodeRateCheck(const CGovernanceObject& govobj, update_mode_enum_t eUpdateLast, bool fForce, bool& fRateCheckBypassed);
    ...
    void CheckSafenodeOrphanVotes();

    void CheckSafenodeOrphanObjects();
...
private:
    ...
    int GetSafenodeIndex(const CTxIn& safenodeVin);

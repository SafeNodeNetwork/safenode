...
/*
    At 15 signatures, 1/2 of the safenode network can be owned by
...
private:
    ...
    //track safenodes who voted with no txreq (for DOS protection)
    std::map<COutPoint, int64_t> mapSafenodeOrphanVotes; // mn outpoint - time
    ...
    int64_t GetAverageSafenodeOrphanVoteTime();
...
public:
    CTxLockVote() :
        ...
        outpoint(),
        outpointSafenode(),
        vchSafenodeSignature(),
        nConfirmedHeight(-1),
        nTimeCreated(GetTime())
        {}

    CTxLockVote(const uint256& txHashIn, const COutPoint& outpointIn, const COutPoint& outpointSafenodeIn) :
        txHash(txHashIn),
        outpoint(outpointIn),
        outpointSafenode(outpointSafenodeIn),
        vchSafenodeSignature(),
        nConfirmedHeight(-1),
        nTimeCreated(GetTime())
        {}
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
        READWRITE(txHash);
        READWRITE(outpoint);
        READWRITE(outpointSafenode);
        READWRITE(vchSafenodeSignature);
    }
    ...
    COutPoint GetSafenodeOutpoint() const { return outpointSafenode; }
...
private:
    ...
    std::map<COutPoint, CTxLockVote> mapSafenodeVotes; // safenode outpoint - vote
...
public:
    ...
    bool HasSafenodeVoted(const COutPoint& outpointSafenodeIn) const;
    int CountVotes() const { return mapSafenodeVotes.size(); }
...
public:
    ...
    bool HasSafenodeVoted(const COutPoint& outpointIn, const COutPoint& outpointSafenodeIn);

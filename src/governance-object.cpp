...
{
    int nMNIndex = governance.GetSafenodeIndex(vote.GetVinSafenode());
    if(nMNIndex < 0) {
        std::ostringstream ostr;
        ostr << "CGovernanceObject::ProcessVote -- Safenode index not found\n";
        exception = CGovernanceException(ostr.str(), GOVERNANCE_EXCEPTION_WARNING);
        if(mapOrphanVotes.Insert(vote.GetVinSafenode(), vote_time_pair_t(vote, GetAdjustedTime() + GOVERNANCE_ORPHAN_EXPIRATION_TIME))) {
            if(pfrom) {
                mnodeman.AskForMN(pfrom, vote.GetVinSafenode());
            }
    ...
    if(governance.AreRateChecksEnabled()) {
        ...
        if(nTimeDelta < GOVERNANCE_UPDATE_MIN) {
            std::ostringstream ostr;
            ostr << "CGovernanceObject::ProcessVote -- Safenode voting too often "
    ...
    if(!mnodeman.AddGovernanceVote(vote.GetVinSafenode(), vote.GetParentHash())) {
        std::ostringstream ostr;
...
void CGovernanceObject::RebuildVoteMap()
{
    ...
    for(vote_m_it it = mapCurrentMNVotes.begin(); it != mapCurrentMNVotes.end(); ++it) {
        CTxIn vinSafenode;
        if(mnodeman.GetSafenodeVinForIndexOld(it->first, vinSafenode)) {
            int nNewIndex = mnodeman.GetSafenodeIndex(vinSafenode);
            if((nNewIndex >= 0)) {
...
void CGovernanceObject::ClearSafenodeVotes()
{
    ...
    while(it != mapCurrentMNVotes.end()) {
        ...
        CTxIn vinSafenode;
        bool fRemove = true;
        if(mnodeman.Get(it->first, vinSafenode, fIndexRebuilt)) {
            if(mnodeman.Has(vinSafenode)) {
                fRemove = false;
            }
            else {
                fileVotes.RemoveVotesFromSafenode(vinSafenode);
            }
        }
...
void CGovernanceObject::SetSafenodeInfo(const CTxIn& vin)
{
    vinSafenode = vin;
}

bool CGovernanceObject::Sign(CKey& keySafenode, CPubKey& pubKeySafenode)
{
    std::string strError;
    ...
    LogPrint("gobject", "CGovernanceObject::Sign -- pubkey id = %s, vin = %s\n",
             pubKeySafenode.GetID().ToString(), vinSafenode.prevout.ToStringShort());


    return true;
}

bool CGovernanceObject::CheckSignature(CPubKey& pubKeySafenode)
{
    std::string strError;
...
{
    bool fMissingSafenode = false;

    return IsValidLocally(strError, fMissingSafenode, fCheckCollateral);
}

bool CGovernanceObject::IsValidLocally(std::string& strError, bool& fMissingSafenode, bool fCheckCollateral)
{
    fMissingSafenode = false;
    ...
    // IF ABSOLUTE NO COUNT (NO-YES VALID VOTES) IS MORE THAN 10% OF THE NETWORK SAFENODES, OBJ IS INVALID

    // CHECK COLLATERAL IF REQUIRED (HIGH CPU USAGE)

    if(fCheckCollateral) { 
        if((nObjectType == GOVERNANCE_OBJECT_TRIGGER) || (nObjectType == GOVERNANCE_OBJECT_WATCHDOG)) {
            std::string strOutpoint = vinSafenode.prevout.ToStringShort();
            safenode_info_t infoMn = mnodeman.GetSafenodeInfo(vinSafenode);
            if(!infoMn.fInfoValid) {
                fMissingSafenode = true;
                strError = "Safenode not found: " + strOutpoint;
                return false;
            }

            // Check that we have a valid MN signature
            if(!CheckSignature(infoMn.pubKeySafenode)) {
                strError = "Invalid safenode signature for: " + strOutpoint + ", pubkey id = " + infoMn.pubKeySafenode.GetID().ToString();
                return false;
            }
...
{
    int nMNIndex = governance.GetSafenodeIndex(mnCollateralOutpoint);
    vote_m_it it = mapCurrentMNVotes.find(nMNIndex);
    if (it == mapCurrentMNVotes.end()) {
        return false;
    }
...
void CGovernanceObject::CheckOrphanVotes()
{
    ...
    while(it != listVotes.end()) {
        ...
        else if(!mnodeman.Has(vote.GetVinSafenode())) {
            ++it;
            continue;
        }
        CGovernanceException exception;

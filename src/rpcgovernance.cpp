...
UniValue gobject(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-alias         - Vote on a governance object by safenode alias (using safenode.conf setup)\n"
                "  vote-conf          - Vote on a governance object by safenode configured in safenode.conf\n"
                "  vote-many          - Vote on a governance object by all safenodes (using safenode.conf setup)\n"
                );
    ...
    */
    ...
    if(strCommand == "prepare")
    {
        ...
        if((govobj.GetObjectType() == GOVERNANCE_OBJECT_TRIGGER) ||
           (govobj.GetObjectType() == GOVERNANCE_OBJECT_WATCHDOG)) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Trigger and watchdog objects need not be prepared (however only safenodes can create them)");
        }
    ...
    if(strCommand == "submit")
    {
        ...
        if(!safenodeSync.IsBlockchainSynced()) {
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Must wait for client to sync with safenode network. Try again in a minute or so.");
        }

        CSafenode mn;
        bool fMnFound = mnodeman.Get(activeSafenode.vin, mn);

        DBG( cout << "gobject: submit activeSafenode.pubKeySafenode = " << activeSafenode.pubKeySafenode.GetHash().ToString()
             << ", vin = " << activeSafenode.vin.prevout.ToStringShort()
        ...
        if((govobj.GetObjectType() == GOVERNANCE_OBJECT_TRIGGER) ||
           (govobj.GetObjectType() == GOVERNANCE_OBJECT_WATCHDOG)) {
            if(fMnFound) {
                govobj.SetSafenodeInfo(mn.vin);
                govobj.Sign(activeSafenode.keySafenode, activeSafenode.pubKeySafenode);
            }
            else {
                LogPrintf("gobject(submit) -- Object submission rejected because node is not a safenode\n");
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Only valid safenodes can submit this type of object");
            }
        }
        else {
            if(params.size() != 6) {
        ...
        if(!governance.SafenodeRateCheck(govobj)) {
        ...
        if(!governance.SafenodeRateCheck(govobj, UPDATE_TRUE)) {
    ...
    if(strCommand == "vote-conf")
    {
        ...
        std::vector<unsigned char> vchSafeNodeSignature;
        ...
        if(!fMnFound) {
            ...
            statusObj.push_back(Pair("errorMessage", "Can't find safenode by collateral output"));
            resultsObj.push_back(Pair("safenode.conf", statusObj));
        ...
        if(!vote.Sign(activeSafenode.keySafenode, activeSafenode.pubKeySafenode)) {
            nFailed++;
            statusObj.push_back(Pair("result", "failed"));
    ...
    if(strCommand == "vote-many")
    {
        ...
        BOOST_FOREACH(CSafenodeConfig::CSafenodeEntry mne, safenodeConfig.getEntries()) {
            ...
            if(!darkSendSigner.GetKeysFromSecret(mne.getPrivKey(), keySafenode, pubKeySafenode)){
                nFailed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", "Safenode signing error, could not set key correctly"));
            ...
            CGovernanceVote vote(mn.vin, hash, eVoteSignal, eVoteOutcome);
            if(!vote.Sign(keySafenode, pubKeySafenode)){
                nFailed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", "Failure to sign."));
                resultsObj.push_back(Pair(mne.getAlias(), statusObj));
    ...
    // SAFENODES CAN VOTE ON GOVERNANCE OBJECTS ON THE NETWORK FOR VARIOUS SIGNALS AND OUTCOMES
    if(strCommand == "vote-alias")
    {
        ...
        // EXECUTE VOTE FOR EACH SAFENODE, COUNT SUCCESSES VS FAILURES
        ...
        BOOST_FOREACH(CSafenodeConfig::CSafenodeEntry mne, safenodeConfig.getEntries())
        {
            ...
            // SETUP THE SIGNING KEY FROM SAFENODE.CONF ENTRY
            ...
            if(!darkSendSigner.GetKeysFromSecret(mne.getPrivKey(), keySafenode, pubKeySafenode)) {
                nFailed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", strprintf("Invalid safenode key %s.", mne.getPrivKey())));
                resultsObj.push_back(Pair(mne.getAlias(), statusObj));
                continue;
            }

            // SEARCH FOR THIS SAFENODE ON THE NETWORK, THE NODE MUST BE ACTIVE TO VOTE
            ...
            if(!fMnFound) {
                nFailed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", "Safenode must be publically available on network to vote. Safenode not found."));
                resultsObj.push_back(Pair(mne.getAlias(), statusObj));
                continue;
            }
    ...
    if(strCommand == "list" || strCommand == "diff")
    {
        ...
        BOOST_FOREACH(CGovernanceObject* pGovObj, objs)
        {
            ...
            const CTxIn& safenodeVin = pGovObj->GetSafenodeVin();
            if(safenodeVin != CTxIn()) {
                bObj.push_back(Pair("SigningSafenode", safenodeVin.prevout.ToStringShort()));
            }
    ...
    if(strCommand == "get")
    {
        ...
        const CTxIn& safenodeVin = pGovObj->GetSafenodeVin();
        if(safenodeVin != CTxIn()) {
            objResult.push_back(Pair("SigningSafenode", safenodeVin.prevout.ToStringShort()));
        }
        ...
        // -- ENDORSED VIA SAFENODE-ELECTED BOARD
    ...
    if(strCommand == "getcurrentvotes")
    {
        ...
        if (params.size() == 4) {
            uint256 txid = ParseHashV(params[2], "Safenode Collateral hash");
...
UniValue voteraw(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 7)
        throw std::runtime_error(
                "voteraw <safenode-tx-hash> <safenode-tx-index> <governance-hash> <vote-signal> [yes|no|abstain] <time> <vote-sig>\n"
    ...
    if(!fMnFound) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failure to find safenode in list : " + vin.prevout.ToStringShort());
    }
...
UniValue getgovernanceinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0) {
        throw std::runtime_error(
            ...
            "  \"safenodewatchdogmaxseconds\": xxxxx,  (numeric) sentinel watchdog expiration time in seconds\n"
    ...
    obj.push_back(Pair("safenodewatchdogmaxseconds", SAFENODE_WATCHDOG_MAX_SECONDS));
